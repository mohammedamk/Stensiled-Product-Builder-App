...
abstract class REST_Controller extends CI_Controller
{
    ...
    protected $_allowed_cors_origins = [];
    ...
    public function __construct($config = 'rest')
    {
        ...
        // Determine supported allowed_origins from configuration
        ...
        $this->_allowed_origins = array_merge(allowed_origins(),$allowed_origins);

        // Determine supported allowed_cors_origins from configuration
        $allowed_cors_origins = $this->config->item('allowed_cors_origins');

        // Validate the configuration setting allowed_cors_origins
        if (!is_array($allowed_cors_origins)) {
            $allowed_cors_origins = [];
        }

        $this->_allowed_cors_origins = array_merge(allowed_cors_origins(),$allowed_cors_origins);
        // print_r($allowed_cors_origins);
        // exit;
        ...
        // if (!in_array($this->request->format, $supported_formats)) {
        //     $this->response([
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        // Remove the supported format from the function name e.g. index.jsonp => index
        $object_called = preg_replace('/^(.*)\.(?:' . implode('|', array_keys($this->_supported_formats)) . ')$/', '$1', $object_called);


        $controller_method = $object_called;

        // _debug($controller_method);
        // exit;
    ...
    protected function _detect_api_key()
    {
        ...
        $_AUTH_USER = $key = $this->input->server('PHP_AUTH_USER');

        if (!array_key_exists($_AUTH_USER, $this->config->item('rest_valid_logins'))) {
            $json_data = json_decode(base64_decode($_AUTH_USER));
            if (!empty($json_data) && count((array) $json_data) > 0 && isset($json_data->shop) && null !== $json_data->shop) {
                if(is_shop_active($json_data->shop)){
                    $shopAccess=getShop_accessToken_byShop($json_data->shop);
                    $this->load->library('Shopify', $shopAccess);
                    $shop = $this->shopify->call(['METHOD' => 'GET', 'URL' =>'/admin/shop.json'], true);
                    if (isset($shop->shop->id)) {
                        $domain = $this->input->server('REQUEST_SCHEME').'://'.$shop->shop->domain;

                        if (!$this->input->server('HTTP_ORIGIN') == $domain) {
                            $this->response([
                                $this->config->item('rest_status_field_name')  => false,
                        ...
                        $PHP_AUTH_USER = $this->config->item('rest_valid_logins');
                        foreach ($PHP_AUTH_USER as $apikey => $value) {
                            $key = $apikey;
                            break;
                        }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        // _debug(base64_decode($digest_string));
        // exit;
    ...
    protected function _check_cors()
    {
        ...
        if ($this->config->item('allow_any_cors_domain') === true) {
            ...
            $check_origin = str_replace($this->input->server('REQUEST_SCHEME').'://', '', $this->input->server('HTTP_ORIGIN'));

            // If the origin domain is in the allowed_cors_origins list, then add the Access Control headers
            if (in_array($check_origin, $this->_allowed_cors_origins)) {
